...
`@smitejs/*`. Each is a small standalone package you can start and hit with
`curl` or a browser.
...
Start it with `yarn workspace @smitejs/example-http-rest-server start`, then:
...
`examples/env-http` wires `@smitejs/env` into a server: declared variables
(`PORT`, `GREETING`) are resolved through a provider and injected into the
handler. Start it with `yarn workspace @smitejs/example-env-http start`.

## Typed client + server

`examples/typed-client` defines an app, generates a typed client with
`@smitejs/client`, and calls it back over HTTP. Build the client with
`yarn workspace @smitejs/example-typed-client build`, start the server with
...
`client()` and `openapi()` plugins, `yarn workspace @smitejs/example-cli-app
...
`examples/fp-utils` exercises `@smitejs/fp` — `Option`, `Result`, `Task`,
`TaskResult`, `chain`, and `flow` — in a small request-handling script. Start it
with `yarn workspace @smitejs/example-fp-utils start`.
