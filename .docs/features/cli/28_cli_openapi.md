# 28. `@smite/openapi` package

## Goal

Stand up a new `@smite/openapi` package delivering the OpenAPI artifact
generator as a CLI plugin: `openapi(options)` traverses the compiled app's
routes via `@smite/http`, converts each `req` bucket's zod schema to JSON
...
`smite.config.ts` (slice 27), not code baked into `@smite/cli`.

The route traversal knowledge lives in `@smite/http` (a `routesOf(app)`
...
### `routesOf(app)` helper (in `@smite/http`)

`@smite/http` exports `routesOf(app: AppDescriptor)` returning:
...
traversal both `@smite/client`'s emit and `@smite/openapi`'s emitter consume.

### `openapi(options)` plugin (in `@smite/openapi`)
...
- `packages/openapi/package.json` — deps `@smite/core`, `@smite/http`,
  `@smite/cli` (for the `SmitePlugin` type + `compileApp` in tests);
  `sideEffects: false`.
- Deps direction: `openapi → http/core/cli`; `@smite/openapi` is never
  imported by any lower package. No cycles.
- Root `tsconfig.build.json` + `vitest.config.ts` get `./packages/openapi`.

## Implementation steps

1. `packages/openapi/package.json` — name `@smite/openapi`, deps
   `@smite/core`, `@smite/http`, `@smite/cli`; `exports` (`.`); `sideEffects:
...
4. `vitest.config.ts` — add `@smite/openapi` alias →
   `packages/openapi/src/index.ts`.
5. `@smite/http` — add `routesOf` to `src/serve.ts` (or a new
...
- `@smite/openapi` adds zero new Biome violations and keeps
  `sideEffects: false`.
...
- Slice `27` (`@smite/cli` foundation: `compileApp`, plugin contract, config
  loader).
- `@smite/http` `serve`/traversal (slices `08`–`11`), zod v4 at root.
...
- **`servers` / `components`**: the emitted doc is minimal. Adding
  ...
  `$defs`) are follow-ups.
- **Client emit parity**: `@smite/client` could later reuse `routesOf` too,
